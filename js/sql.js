function TournamentDb () {
	this.players = [];
	this.db = openDatabase('Tournamenwdawdawdawdt12111', '1.0', 'Tournament database', 2 * 1024 * 1024);
	this.db.transaction(function (tx){
  		tx.executeSql('CREATE TABLE IF NOT EXISTS players (id unique, name, team, gols, points)');
  		tx.executeSql('CREATE TABLE IF NOT EXISTS games (id unique, player1, player2, resulP1, resulP2)');
	});
}
...
TournamentDb.prototype.addGames = function(id, player1, player2) {
	this.db.transaction(function (tx) {
		tx.executeSql('INSERT INTO games (id, player1, player2) VALUES ('+id+'"'+player1+'", "'+player2+'",'+resulP1+','+resulP2+')');
	});
};
